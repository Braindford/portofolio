...
const mabar = document.getElementById('mabar')
...
window.addEventListener('scroll', () => {
    ...
    if (window.scrollY >= html.offsetHeight) {
        htmlBar.style.width = '40%'
    }
    if (window.scrollY >= css.offsetHeight) {
        cssBar.style.width = '30%'
    }
    ...
    if (window.scrollY >= mabar.offsetHeight) {
        mabar.style.opacity = 1
        mabar.style.transition = '1.8s'
        mabar.style.transform = 'translateY(0)'
    }
